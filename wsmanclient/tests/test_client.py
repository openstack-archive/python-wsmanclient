...
class ClientTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = client.Client(**test_utils.FAKE_ENDPOINT)
    ...
    def test_enumerate_with_request_failure(self):
        self.client = client.Client('malformed://^@*', 'user', 'pass')

        self.assertRaises(exceptions.RequestFailure,
    ...
    def test_enumerate_with_auto_pull(self, mock_requests):
        ...
        self.assertEqual(
            0, len(resp_xml.findall(
                './/{%s}EnumerationContext' % client.NS_WSMAN_ENUM)))

    @requests_mock.Mocker()
    @mock.patch.object(client.Client, 'pull', autospec=True)
...
class PayloadTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        client.NS_MAP = collections.OrderedDict([
            ('s', client.NS_SOAP_ENV),
            ('wsa', client.NS_WS_ADDR),
            ('wsman', client.NS_WSMAN)])
...
"""  # noqa
        ...
        payload = client._EnumeratePayload('http://host:443/wsman',
                                           'http://resource_uri').build()
...
"""  # noqa
    ...
    def test_build_enum_with_invalid_filter_dialect(self):
        invalid_dialect = 'foo'
        self.assertRaises(exceptions.InvalidFilterDialect,
                          client._EnumeratePayload,
                          'http://host:443/wsman', 'http://resource_uri',
...
"""  # noqa
        ...
        mock_uuid.return_value = '1234-12'
        payload = client._PullPayload('http://host:443/wsman',
                                      'http://resource_uri',
                                      'context-uuid').build()
        payload_obj = lxml.objectify.fromstring(payload)
